...
def add_file():
    files = filedialog.askopenfilenames(title="파일을 선택하세요.", filetypes=(("PNG 파일", "*.png"), ("모든 파일", "*.*")), initialdir="C:/")
...
def merge_image():
    images = [Image.opne(x) for x in list.file.get(0, END)]
...
file_frame.pack(fill="x", padx=5, pady=5) # padx, pady 는 버튼 바깥여백 ipadx, ipady는 버튼 안쪽 여백
...
p_var = DoubleVar()
progressbar_bar = ttk.Progressbar(frame_progress, maximum=100, variable=p_var)
